...
<form enctype="multipart/form-data" action="controller/updateRecord.php" method="post" class="d-flex flex-column gap-3">
  ...
  <div class="container p-0 d-flex flex-column gap-2">
    ...
    <?php
        ...
        ?>
        <img id="preview" src="<?= $fileData['avatar'] ? htmlspecialchars($fileData['avatar']) : '' ?>" class="mb-2"
            style="width:<?= $fileData['width'] ?>px;<?= $fileData['avatar'] ? '' : 'display:none' ?>; max-height: 200px; object-fit: contain; border-radius: .25rem">
    </div>
